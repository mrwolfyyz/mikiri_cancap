...
(function (global) {
    ...
    function extractTabId(pageName) {
        ...
        const tabMap = {
            'identity': 'identity', 'skiptrace': 'skiptrace', 'regulator': 'regulator',
            'corporate': 'corporate', 'adverse_media': 'adverse_media',
            'borrower_summary': 'identity', 'summary': 'summary'
        };
    ...
    function getCalloutIcon(type) {
        const icons = { 'abstract': '📋', 'danger': '🔴', 'warning': '⚠️', 'info': 'ℹ️', 'note': '📝', 'tip': '💡' };
    ...
    function renderMarkdown(container, markdown, reportType, jobId) {
        ...
        if (typeof marked === 'undefined') return '<p>Markdown renderer not loaded.</p>';
        ...
        renderer.listitem = function (text, task, checked) {
            if (task === true) {
                ...
                return `<li class="task-list-item">
                    <input type="checkbox" ${isChecked ? 'checked' : ''} data-task-id="${taskId}" data-job-id="${jobId}" />
    ...
    function getTabsForWorkflow(markdownReports) {
        ...
        if (markdownReports.skiptrace) {
            ...
            tabs.push({ id: 'skiptrace', label: 'Skip Trace Checklist', badge: taskCount > 0 ? taskCount : null });
        }
        if (markdownReports.corporate) tabs.push({ id: 'corporate', label: 'Corporate' });
        if (markdownReports.adverse_media) tabs.push({ id: 'adverse_media', label: 'Adverse Media' });
        if (markdownReports.regulator) tabs.push({ id: 'regulator', label: 'Regulator' });
        return tabs;
    }
    ...
    async function loadMarkdownReports(apiUrl, jobId, getToken) {
        ...
        if (!response.ok) {
            ...
            throw new Error(err.error || `Failed to load reports: ${response.status}`);
        }
    ...
    function renderReport(container, options) {
        ...
        const tabsHtml = tabs.map((tab, i) => `
            <button class="tab-button ${i === 0 ? 'active' : ''}" data-tab="${tab.id}">
                ${tab.label}${tab.badge ? ` <span class="tab-badge">${tab.badge}</span>` : ''}
            </button>
        `).join('');

        const panelsHtml = tabs.map((tab, i) => {
            const md = markdownReports[tab.id] || `# ${tab.label}\n\nNo data available.`;
            const html = renderMarkdown(container, md, tab.id, jobId);
            return `<div class="tab-panel ${i === 0 ? 'active' : ''}" data-tab="${tab.id}">
        ...
        container.innerHTML = `
            <div class="report-header">
                <div class="report-info">
                    <h2 id="reportTitle">${escapeHtml(name)}${city ? ' - ' + escapeHtml(city) : ''}</h2>
        ...
        function switchTab(tabId) {
            container.querySelectorAll('.tab-button').forEach(btn => btn.classList.toggle('active', btn.dataset.tab === tabId));
            container.querySelectorAll('.tab-panel').forEach(p => p.classList.toggle('active', p.dataset.tab === tabId));
        }
        ...
        if (isSkiptrace) {
            ...
            const savedToggle = global.localStorage.getItem('skipTraceChecklistVisible');
            if (savedToggle !== null) checklistToggle.checked = savedToggle === 'true';
            ...
            function applyVisibility() {
                ...
                if (!visible && panel?.classList.contains('active') && firstTab) switchTab(firstTab);
            }
            ...
            function updateProgress() {
                const cbs = container.querySelectorAll('.task-list-item input[type="checkbox"]');
                const total = cbs.length;
                const completed = Array.from(cbs).filter(cb => cb.checked).length;
                progressEl.textContent = `${completed}/${total} completed (${total ? Math.round(completed / total * 100) : 0}%)`;
            }
            ...
            resetBtn.addEventListener('click', () => {
                ...
                container.querySelectorAll('.task-list-item input[type="checkbox"]').forEach(cb => { cb.checked = false; });
                updateProgress();
            });
        } else {
            ...
            if (dangerCount > 0) { riskLevel = 'High'; riskClass = 'high'; }
            else if (warningCount > 2) { riskLevel = 'High'; riskClass = 'high'; }
            else if (warningCount > 0) { riskLevel = 'Medium'; riskClass = 'medium'; }
            ...
            const hasDanger = container.querySelector('.callout-danger');
            const hasWarning = container.querySelector('.callout-warning');
            container.querySelectorAll('.button-secondary').forEach(btn => {
                btn.classList.remove('button-danger', 'button-warning');
                if (hasDanger) btn.classList.add('button-danger');
                else if (hasWarning) btn.classList.add('button-warning');
            });
        }
    }
