# Complaints Search Engine (COMPLAINTS_PSE_CX)

## Overview

This Programmable Search Engine (PSE) is configured for business complaint searches, used in address verification workflows.

## Configuration Details

- **CX ID**: `f55c8831c767349da`
- **Region**: Canada
- **Engine Identifier**: `business_complaints`
- **Public URL**: `https://cse.google.com/cse?cx=f55c8831c767349da`

## Environment Variable

- **Variable Name**: `COMPLAINTS_PSE_CX`
- **Usage**: Set this environment variable to the CX ID that Google generated when you created this PSE

## Site Restrictions

- **No site restrictions** - searches the entire web
- "You do not have sites to search" (unrestricted search)

## Search Features

- **Image search**: Disabled (OFF)
- **SafeSearch**: Disabled (OFF)
- **Search the entire web**: Enabled (ON)
- **Region restricted results**: Disabled (OFF)

## Query Enhancement

This PSE has synonyms configured for query enhancement:

- **Search term**: `complaints`
- **Synonyms**:
  - `issues`
  - `scam`
  - `fraud`
  - `lawsuit`
  - `investigation`

When searching for "complaints", the PSE will also search for these related terms automatically.

## Usage

This PSE is used by:

- **address_verification** Cloud Function
  - Performs business complaint searches to verify business addresses
  - Used to find complaints, issues, fraud reports, and lawsuits related to businesses
  - Helps identify potential fraud or legitimacy concerns

## Manual Setup Instructions

When creating this PSE in Google PSE control panel:

1. Create a new Programmable Search Engine
2. Name it "Complaints Search" or "Business Complaints" or similar
3. Set region to **Canada**
4. Disable **Image search**
5. Disable **SafeSearch**
6. Enable **Search the entire web**
7. Disable "Region restricted results"
8. **Do not add any site restrictions** - leave "Sites to search" empty (searches entire web)
9. **Configure Query Enhancement** (under Search Features → Query Enhancement):
   - Add search term: `complaints`
   - Add synonyms: `issues`, `scam`, `fraud`, `lawsuit`, `investigation`
10. Save the PSE and copy the Search Engine ID (CX value) - Google will generate a new CX ID for your PSE
11. Set the `COMPLAINTS_PSE_CX` environment variable to the CX ID that Google generated

## Notes

- This PSE is specifically designed for finding business complaints and fraud indicators
- The query enhancement synonyms help expand search coverage for related legal and fraud-related terms
- Unrestricted web search allows discovery across all complaint platforms and legal databases
- Used as part of address verification workflow in the origination frontend to identify potential fraud risks
