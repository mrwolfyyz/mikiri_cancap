...
# -----------------------------------------------------------------------------
# Check Frontend Shared Files (generated by prepare-frontend.sh)
# -----------------------------------------------------------------------------
echo "Checking frontend shared files (from prepare-frontend.sh)..."
...
for platform in skiptrace origination; do
  for file in "${FRONTEND_SHARED_FILES[@]}"; do
    if [[ -f "$ROOT_DIR/frontend/$platform/public/$file" ]]; then
      echo "  ✓ frontend/$platform/public/$file"
    else
      echo "  ❌ frontend/$platform/public/$file - NOT FOUND (run scripts/prepare-frontend.sh)"
      ALL_OK=false
    fi
  done
done

# Check address-verification.js only for origination
if [[ -f "$ROOT_DIR/frontend/origination/public/address-verification.js" ]]; then
  echo "  ✓ frontend/origination/public/address-verification.js"
else
  echo "  ❌ frontend/origination/public/address-verification.js - NOT FOUND"
  ALL_OK=false
fi
