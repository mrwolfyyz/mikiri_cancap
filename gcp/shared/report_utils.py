...
def check_domain_mx_records(domain: str) -> Dict[str, Any]:
    ...
    results = {
        "success": False, "domain": domain, "status": "Unknown",
        "provider_detected": None, "mx_records": [], "risk_level": "UNKNOWN", "error": None
    }
    ...
    try:
        ...
        for sig, name in HIGH_TRUST_PROVIDERS.items():
            if sig in primary_mx:
                results.update({"success": True, "status": "Legitimate Business Email", "provider_detected": name, "risk_level": "LOW"})
                print(f"    ✓ MX lookup successful: {domain} uses {name}")
                return results

        for sig, name in STANDARD_TRUST_PROVIDERS.items():
            if sig in primary_mx:
                results.update({"success": True, "status": "Standard Business Email", "provider_detected": name, "risk_level": "LOW/MEDIUM"})
                print(f"    ✓ MX lookup successful: {domain} uses {name}")
                return results

        for sig, name in LOW_TRUST_FLAGS.items():
            if sig in primary_mx:
                results.update({"success": True, "status": "Registrar Default / Parked", "provider_detected": name, "risk_level": "HIGH"})
                print(f"    ⚠️  MX lookup: {domain} uses {name}")
                return results

        if domain in primary_mx:
            results.update({"success": True, "status": "Self-Hosted / Local Hosting", "provider_detected": "Private Server (e.g., cPanel/Exchange)", "risk_level": "MEDIUM"})
            print(f"    ✓ MX lookup: {domain} uses self-hosted email")
            return results

        results.update({"success": True, "status": "Unknown Email Provider", "provider_detected": f"Unrecognized provider: {primary_mx}", "risk_level": "MEDIUM"})
