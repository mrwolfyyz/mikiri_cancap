...
def get_navigation_bar(data: Dict[str, Any], name: str, current_report: str) -> str:
    ...
    # Skiptrace is always available if report exists
    skiptrace_status = "🟢 Skiptrace"
    ...
    nav_items = {
        ...
        'skiptrace': f"[[Skiptrace___{wiki_name}|{skiptrace_status}]]",
    }
...
> {nav_items['identity']} • {nav_items['regulator']} • {nav_items['corporate']} • {nav_items['litigation']} • {nav_items['skiptrace']}
...
# ------------------------------
# Skiptrace Operational Checklist
# ------------------------------

def get_handle_snippet(handle_url: str, queries: List[Dict]) -> Optional[str]:
    """Extract snippet for a handle URL from query hits."""
    for query in queries:
        for hit in query.get('hits', []):
            hit_url = hit.get('url', '').lower()
            if (handle_url.lower() == hit_url or 
                (hit_url and hit_url.rstrip('/') == handle_url.lower().rstrip('/'))):
                return hit.get('snippet', '')
    return None


def has_facebook_profile(handles: List[Dict]) -> bool:
    """Check if any handle is a Facebook profile."""
...
def get_facebook_profile_url(handles: List[Dict]) -> Optional[str]:
    """Get the URL of the first Facebook profile found."""
    for handle in handles:
        platform = handle.get('platform', '').lower()
        url = handle.get('url', '')
        if platform == 'facebook' or 'facebook.com' in url.lower():
            return url
    return None


def has_gravatar_profile(email: str) -> Optional[Dict[str, Any]]:
    """Check if Gravatar profile exists for email. Returns result dict if found, None otherwise."""
    if not email:
        return None
    domain = extract_email_domain(email)
    if not domain or not is_personal_email_domain(domain):
        return None
    gravatar_result = get_gravatar_profile(email)
    if gravatar_result and gravatar_result.get('success'):
...
def find_facebook_public_url(queries: List[Dict]) -> Optional[str]:
    """Find the first facebook.com/public URL in query hits."""
    for query in queries:
        for hit in query.get('hits', []):
            url = hit.get('url', '')
            if url and 'facebook.com/public' in url.lower():
                return url
    return None


def generate_skiptrace_checklist(data: Dict[str, Any], name: str, output_dir: Path, corporate_addresses: Optional[List[Dict[str, Any]]] = None) -> None:
    """Generate the Skiptrace Operational Checklist markdown file.
    
    Args:
        data: Investigation data
        name: Borrower name
        output_dir: Output directory for markdown file
        corporate_addresses: Optional list of pre-extracted corporate addresses (from corporate_contact_info).
                            Each address dict should have 'address_raw', 'source_url', 'snippet', and optionally 'confidence'.
                            If provided, these will be combined with identity addresses before deduplication.
    """
    ...
    # Filter high-confidence handles
    high_confidence_handles = [h for h in top_handles if h.get('confidence', '').lower() == 'high']
    
    # Filter medium/low confidence Facebook handles
    medium_low_facebook_handles = [
        h for h in top_handles 
        if h.get('confidence', '').lower() in ['medium', 'low']
        and (h.get('platform', '').lower() == 'facebook' or 'facebook.com' in h.get('url', '').lower())
    ]
    
    # Check for Gravatar profile
    gravatar_result = has_gravatar_profile(email) if email else None
    ...
    wiki_name = name.replace(' ', '_')
    content = header
    content += get_navigation_bar(data, name, 'skiptrace')
    content += "# Skiptrace: Operational Checklist\n\n"
    content += f"*Generated: {report_timestamp}*\n\n"
    content += "---\n\n"
    
    # Section 1: Investigate Each High Confidence Handle
    content += "## Investigate Each High Confidence Handle\n\n"
    
    # Track handles we've already checked against 500 sites to avoid duplicates
    handles_checked_500_sites = set()
    
    if not high_confidence_handles:
        content += "> [!info] No High-Confidence Handles\n"
        content += "> No high-confidence handles found. Review identity note for lower confidence hits.\n\n"
    else:
        for handle in high_confidence_handles:
            platform = handle.get('platform', 'Unknown')
            handle_name = handle.get('handle', '')
            url = handle.get('url', '')
            
            content += f"### {platform} — `{handle_name}`\n\n"
            
            # Checkbox 1: Investigate profile
            content += f"- [ ] [**Investigate profile**]({url})\n"
            
            # Add snippet if available
            snippet = get_handle_snippet(url, queries)
            if snippet:
                content += f"  > {snippet}\n"
            content += "\n"
            
            # Checkbox 2: Search against 500 sites (only if we haven't seen this handle before)
            if handle_name not in handles_checked_500_sites:
                whatsmyname_url = f"https://whatsmyname.app/?q={quote_plus(handle_name)}"
                content += f"- [ ] [**Search discovered handle against 500 sites**]({whatsmyname_url})\n\n"
                handles_checked_500_sites.add(handle_name)
            
            # Checkbox 3: Facebook marketplace (conditional)
            is_facebook_handle = platform.lower() == 'facebook' or 'facebook.com' in url.lower()
            if is_facebook_handle:
                content += f"- [ ] [**Check for hidden Facebook marketplace ID**]({url})\n\n"
            
            content += "---\n\n"
    
    # Section: Review Medium/Low Confidence Facebook Handles
    if medium_low_facebook_handles:
        content += "## Review Medium/Low Confidence Facebook Handles\n\n"
        
        for handle in medium_low_facebook_handles:
            platform = handle.get('platform', 'Unknown')
            handle_name = handle.get('handle', '')
            url = handle.get('url', '')
            confidence = handle.get('confidence', 'medium').title()
            
            content += f"### {platform} — `{handle_name}` ({confidence} confidence)\n\n"
            ...
            # Checkbox 2: Facebook marketplace
            content += f"- [ ] [**Check for hidden Facebook marketplace ID**]({url})\n\n"
            
            content += "---\n\n"
    
    # Gravatar section (if profile exists)
    if gravatar_result and gravatar_result.get('profile_url'):
        content += "## Review Gravatar Profile\n\n"
        gravatar_url = gravatar_result['profile_url']
        content += f"- [ ] [**Review Gravatar profile for additional profiles**]({gravatar_url})\n\n"
        content += "---\n\n"
    
    # Check for non-indexed Facebook account section
    # Only show if there's no high-confidence Facebook handle but a facebook.com/public URL exists
    # AND the URL is not already displayed in high-confidence or medium/low confidence sections
    has_high_confidence_facebook = False
    displayed_facebook_urls = set()
    
    # Collect all Facebook URLs from high-confidence handles
    for handle in high_confidence_handles:
        platform = handle.get('platform', '').lower()
        url = handle.get('url', '')
        if platform == 'facebook' or 'facebook.com' in url.lower():
            has_high_confidence_facebook = True
            # Normalize URL for comparison (lowercase, strip trailing slash)
            normalized_url = url.lower().rstrip('/')
            displayed_facebook_urls.add(normalized_url)
    
    # Collect all Facebook URLs from medium/low confidence handles
    for handle in medium_low_facebook_handles:
        url = handle.get('url', '')
        # Normalize URL for comparison (lowercase, strip trailing slash)
        normalized_url = url.lower().rstrip('/')
    ...
    facebook_public_url = find_facebook_public_url(queries)
    
    # Check if facebook_public_url is already displayed
    is_duplicate = False
    if facebook_public_url:
        normalized_public_url = facebook_public_url.lower().rstrip('/')
        is_duplicate = normalized_public_url in displayed_facebook_urls
    
    if not has_high_confidence_facebook and facebook_public_url and not is_duplicate:
        content += "## Review Non-Indexed Facebook account\n\n"
        content += f"- [ ] [**Investigate relevant profiles**]({facebook_public_url})\n"
    ...
    # Combine identity addresses with corporate addresses if provided
    if corporate_addresses:
        print(f"[Skip Trace] Combining {len(identity_addresses)} identity addresses with {len(corporate_addresses)} corporate addresses")
        all_addresses = identity_addresses + corporate_addresses
    else:
        all_addresses = identity_addresses
    
    # Deduplicate addresses for skiptrace report
    # Even though extract_addresses_from_queries has deduplication, add an extra layer
    # to ensure no duplicates appear in the skiptrace checklist
    seen_addresses = set()
    unique_addresses = []
    
    for addr_data in all_addresses:
        normalized = normalize_address(addr_data['address_raw'])
        if normalized not in seen_addresses:
            seen_addresses.add(normalized)
            unique_addresses.append(addr_data)
    
    if unique_addresses:
        content += "## Investigate Address(es) of Interest\n\n"
        
        for i, addr_data in enumerate(unique_addresses, start=1):
            raw_addr = addr_data['address_raw']
            
            content += f"### Address {i}: {raw_addr}\n\n"
            
            # Display source and snippet if available
            if addr_data.get("source_url"):
                content += f"**Source:** {addr_data['source_url']}  \n"
            if addr_data.get("snippet"):
                content += "**Snippet:**  \n"
                content += f"> {addr_data['snippet']}\n"
            if addr_data.get("source_url") or addr_data.get("snippet"):
                content += "\n"
            
            # Checkbox 1: Reverse address search (Canada411)
            canada411_url = generate_canada411_url(addr_data)
            content += f"- [ ] [**Perform a reverse address search**]({canada411_url})\n\n"
            
            # Checkbox 2: Google search
            google_url = generate_google_search_url(addr_data)
            content += f"- [ ] [**Perform a google search**]({google_url})\n\n"
            
            # Checkbox 3: Google document search
            google_doc_url = generate_google_doc_search_url(addr_data)
            content += f"- [ ] [**Perform a google document search**]({google_doc_url})\n\n"
            
            content += "---\n\n"
    
    # Section: Investigate Phone Numbers of Interest
    phones = contact_info.get('phones', [])
    
    # Deduplicate phones (similar to addresses)
    seen_phone_digits = set()
    unique_phones = []
    
    for phone_data in phones:
        digits = phone_data.get('number_digits', '')
        if digits and digits not in seen_phone_digits:
            seen_phone_digits.add(digits)
            unique_phones.append(phone_data)
    
    if unique_phones:
        content += "## Investigate Phone Numbers of Interest\n\n"
        
        for i, phone_data in enumerate(unique_phones, start=1):
            number_raw = phone_data.get('number_raw', '')
            
            content += f"### Phone Number {i}: {number_raw}\n\n"
            
            # Display source and snippet if available
            if phone_data.get("source_url"):
                content += f"**Source:** {phone_data['source_url']}  \n"
            if phone_data.get("snippet"):
                content += "**Snippet:**  \n"
                content += f"> {phone_data['snippet']}\n"
            if phone_data.get("source_url") or phone_data.get("snippet"):
                content += "\n"
            
            # Checkbox 1: Google search with variations
            google_url = generate_google_search_url_for_phone(phone_data)
            content += f"- [ ] [**Search google**]({google_url})\n\n"
            
            # Checkbox 2: Google document search
            google_doc_url = generate_google_doc_search_url_for_phone(phone_data)
            content += f"- [ ] [**Search google for documents**]({google_doc_url})\n\n"
            
            content += "---\n\n"
    
    # Section: Investigate Emails of Interest
    emails = contact_info.get('emails', [])
    ...
    if unique_emails:
        content += "## Investigate Emails of Interest\n\n"
        
        # Track handles we've already checked against 500 sites to avoid duplicates
        handles_checked_500_sites_emails = set()
        
        for i, email_data in enumerate(unique_emails, start=1):
            email_addr = email_data.get('email', '')
            
            content += f"### Email {i}: `{email_addr}`\n\n"
            
            # Display source and snippet if available
            if email_data.get("source_url"):
                content += f"**Source:** {email_data['source_url']}  \n"
            if email_data.get("snippet"):
                content += "**Snippet:**  \n"
                content += f"> {email_data['snippet']}\n"
            if email_data.get("source_url") or email_data.get("snippet"):
                content += "\n"
            
            # Extract handle from email
            handle = extract_email_handle(email_addr)
            
            # Checkbox 1: Search handle against 500 sites (only if we haven't seen this handle before)
            if handle and handle not in handles_checked_500_sites_emails:
                whatsmyname_url = f"https://whatsmyname.app/?q={quote_plus(handle)}"
                content += f"- [ ] [**Search handle against 500 sites**]({whatsmyname_url})\n\n"
                handles_checked_500_sites_emails.add(handle)
            
            # Checkbox 2: Google search
            google_url = generate_google_search_url_for_email(email_addr)
            content += f"- [ ] [**Search google**]({google_url})\n\n"
            
            # Checkbox 3: Google document search
            google_doc_url = generate_google_doc_search_url_for_email(email_addr)
            content += f"- [ ] [**Search google for documents**]({google_doc_url})\n\n"
            
            content += "---\n\n"
    
    # Section 2: Review Identity Note for Other Clues
    content += "## Review Identity Note for Other Clues\n\n"
    
    # Checkbox 1: Review lower confidence hits
    identity_link = f"[[Identity___{wiki_name}|Identity Note]]"
    content += f"- [ ] **Review lower confidence hits** — {identity_link}\n"
    content += "  > Review handles with medium or low confidence in the Identity note.\n\n"
    
    # Checkbox 2: Review all search results
    content += f"- [ ] **Review all search results** — {identity_link}#sources\n"
    content += "  > Review all search queries and hits in the Sources section of the Identity note.\n\n"
    
    content += "---\n\n"
    
    output_path = output_dir / f"Skiptrace___{name.replace(' ', '_')}.md"
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"Generated: {output_path}")
...
def get_navigation_bar_skiptrace(data: Dict[str, Any], name: str, current_report: str) -> str:
    """
    Generate a navigation bar for skip trace reports showing only Identity and Skip Trace.
    current_report should be one of: 'identity', 'skiptrace'
    """
    wiki_name = name.replace(' ', '_')
    ...
    nav_bar = f"""> [!abstract] -
-> {nav_items['identity']} • {nav_items['skiptrace']}
...
def generate_skiptrace_checklist_skiptrace(data: Dict[str, Any], name: str, output_dir: Path, enrichment_data: Dict[str, Any] = None) -> None:
    """
    Generate the Skiptrace Operational Checklist markdown file for skip trace (no corporate addresses).
    
    Args:
        data: Investigation data
        name: Borrower name
        output_dir: Output directory for markdown file
        enrichment_data: Optional pre-fetched enrichment data with 'addresses' key
    """
    ...
    wiki_name = name.replace(' ', '_')
    content = header
    content += get_navigation_bar_skiptrace(data, name, 'skiptrace')
    content += "# Skiptrace: Operational Checklist\n\n"
    content += f"*Generated: {report_timestamp}*\n\n"
    content += "---\n\n"
    ...
    # Also get addresses from enrichment_data if available (geocoding results)
    enrichment_addresses = []
    if enrichment_data and enrichment_data.get('addresses'):
    ...
    # Combine identity addresses from LLM extraction and enrichment addresses
    # NOTE: NO corporate_addresses parameter - removed for skip trace
    all_addresses = identity_addresses + enrichment_addresses
    print(f"[Skip Trace] Using {len(identity_addresses)} identity addresses + {len(enrichment_addresses)} enrichment addresses (no corporate addresses for skip trace)")
    
    # Deduplicate addresses for skiptrace report and filter city-only addresses
    # Even though extract_addresses_from_queries has deduplication, add an extra layer
    # to ensure no duplicates appear in the skiptrace checklist
    seen_addresses = set()
    unique_addresses = []
    ...
    for addr_data in all_addresses:
        ...
        # Additional validation: Filter out city-only addresses as a safety check
        # Look for street number pattern (digit at start) or street name indicators
        has_street_number = bool(re.search(r'^\d{1,6}\s+[A-Za-z]', addr_raw))
        # Check for common street name patterns (Avenue, Street, Road, etc. preceded by text)
        has_street_name = bool(re.search(r'\b([A-Za-z0-9.\-\s]+?(?:Avenue|Street|Road|Lane|Drive|Boulevard|Way|Court|Place|Crescent|Circle|Terrace|Parkway|Highway|Ave|St|Rd|Dr|Blvd|Ln|Way|Ct|Pl|Cres|Cir|Terr|Pkwy|Hwy))\b', addr_raw, re.IGNORECASE))
        
        if not has_street_number and not has_street_name:
            # This appears to be a city-only address, skip it
            print(f"[Skip Trace Report] Filtered out city-only address: {addr_raw}")
            continue
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"[Skip Trace] Generated: {output_path}")
