    ...
    if domain and not is_personal_email_domain(domain):
        ...
        if need_whois or need_mx:
            ...
            with ThreadPoolExecutor(max_workers=2) as executor:
                futures = {}
                if need_whois:
                    print(f"[Identity Report SkipTrace] WARNING: Falling back to inline whois lookup for {domain} (domain_enrichment data missing)")
                    futures[executor.submit(get_domain_registration_date, domain)] = 'whois'
                if need_mx:
                    print(f"[Identity Report SkipTrace] WARNING: Falling back to inline MX lookup for {domain} (domain_enrichment data missing)")
    ...
    if company_domain:
        ...
        if company_domain:
            ...
            if enrichment_data and enrichment_data.get('domains') and company_domain in enrichment_data['domains']:
                ...
                from concurrent.futures import ThreadPoolExecutor, as_completed
                print(f"[Identity Report SkipTrace] WARNING: Falling back to inline lookups for company domain {company_domain} (domain_enrichment data missing)")
                with ThreadPoolExecutor(max_workers=2) as executor:
