...
def transform_firestore_to_report_format(firestore_data: dict[str, Any]) -> dict[str, Any]:
    """
    Transform Firestore result structure to match generate_markdown_reports_skiptrace.py expectations.

    Skip trace Firestore structure:
...
def find_or_create_folder(drive_service, parent_folder_id: str, folder_name: str) -> str:
    ...
    Raises HttpError if parent folder doesn't exist or is inaccessible.
    """
    # First, validate that the parent folder exists
    try:
        parent = (
            drive_service.files()
            .get(fileId=parent_folder_id, fields="id, name, mimeType", supportsAllDrives=True)
            .execute()
        )
        if parent.get("mimeType") != "application/vnd.google-apps.folder":
            raise ValueError(f"Parent ID {parent_folder_id} is not a folder")
    except HttpError as e:
        if e.resp.status == 404:
            raise HttpError(
                resp=e.resp,
                content=f"Parent Drive folder not found: {parent_folder_id}. Please check that the folder ID is correct and accessible.",
                uri=e.uri,
            ) from e
        raise
...
def on_job_updated(event: CloudEvent) -> None:
    ...
    Only processes jobs with workflow_type == "skiptrace".
    """
    print("[SkipTraceReportGenerator] Function triggered!")
    job_id = None

    try:
        # Parse Firestore event from CloudEvent
        attributes = event.get_attributes() if hasattr(event, "get_attributes") else {}
        print(f"[SkipTraceReportGenerator] Event attributes: {attributes}")

        # Extract job_id from document attribute
        document_path = attributes.get("document", "")
        if not document_path:
            print("[SkipTraceReportGenerator] ERROR: 'document' attribute not found in event")
            return
        ...
        if "/jobs/" in document_path:
            ...
            print(f"[SkipTraceReportGenerator] ERROR: Could not extract job_id from document path: {document_path}")
            return
        ...
        try:
            ...
            if payload_workflow and payload_workflow != "skiptrace":
                print(
                    f"[SkipTraceReportGenerator] Early exit: workflow_type is '{payload_workflow}' (from event payload)"
                )
                return
        ...
        doc = doc_ref.get()
        if not doc.exists:
            print("[SkipTraceReportGenerator] ERROR: Document not found in Firestore")
            return

        doc_data = doc.to_dict()
        if not doc_data:
            print("[SkipTraceReportGenerator] ERROR: Document data is empty")
            return

        # Check workflow_type - only process skip trace workflows
        workflow_type = doc_data.get("workflow_type", "")
        if workflow_type != "skiptrace":
            print(f"[SkipTraceReportGenerator] Skipping: workflow_type is '{workflow_type}', expected 'skiptrace'")
            return
        ...
        if status != "post_processing":
            print(f"[SkipTraceReportGenerator] Skipping: status is '{status}', expected 'post_processing'")
            return
        ...
        if isinstance(result_str, str):
            try:
                ...
                print(f"[SkipTraceReportGenerator] ERROR: Failed to parse result JSON: {e}")
                return
        else:
        ...
        print(f"[SkipTraceReportGenerator] Generating skip trace reports for: {borrower_name}")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            md_gen.generate_identity_report_skiptrace(
                report_data, borrower_name, output_dir, company_domain=company_domain, enrichment_data=enrichment_data
            )

            print("[SkipTraceReportGenerator] ✓ All reports generated")

            # Upload to Google Drive
            reports_folder_link = None
            if not drive_folder_id:
                print("[SkipTraceReportGenerator] WARNING: No drive_folder_id provided, skipping Drive upload")
                drive_urls = {}
            else:
                try:
                    print(f"[SkipTraceReportGenerator] Uploading to Google Drive folder: {drive_folder_id}")
                    drive_service = get_drive_service()
                    ...
                    if not files_to_upload:
                        print("[SkipTraceReportGenerator] WARNING: No files found to upload")
                    else:
                        print(f"[SkipTraceReportGenerator] Uploading {len(files_to_upload)} files in parallel...")
                        ...
                        with ThreadPoolExecutor(max_workers=2) as executor:
                            ...
                            for future in as_completed(future_to_file):
                                ...
                                try:
                                    print(f"[SkipTraceReportGenerator] Uploading {file_name}...")
                                    _, file_meta = future.result()
                                    report_urls[file_name] = file_meta.get("webViewLink", "")
                                    print(f"[SkipTraceReportGenerator] ✓ Uploaded {file_name}")
                                except Exception as e:
                                    error_msg = f"Failed to upload {file_name}: {str(e)}"
                                    print(f"[SkipTraceReportGenerator] ERROR: {error_msg}")
                                    upload_errors.append(error_msg)

                        # Log warning for missing files
                        missing_files = set(report_files) - {f[0] for f in files_to_upload}
                        for file_name in missing_files:
                            print(f"[SkipTraceReportGenerator] WARNING: File not found: {output_dir / file_name}")
                    ...
                    print(f"[SkipTraceReportGenerator] ✓ All reports uploaded to Drive ({len(report_urls)} files)")
                except HttpError as e:
                    ...
                    print(f"[SkipTraceReportGenerator] WARNING: {error_msg}")
            ...
            try:
                if identity_file.exists():
                    ...
                    print(
                        f"[SkipTraceReportGenerator] ✓ Read Identity markdown ({len(markdown_reports['identity'])} chars)"
                    )
                else:
            ...
            print(f"[SkipTraceReportGenerator] ✓ Job {job_id} completed successfully")

    except Exception as e:
        print(f"[SkipTraceReportGenerator] ERROR: {e}")
        import traceback
        ...
        try:
            ...
            print(f"[SkipTraceReportGenerator] ERROR: Failed to update Firestore with error: {update_error}")
