...
def retry_with_backoff(
    fn: Callable[[], T], config: RetryConfig = RetryConfig(), operation_name: str = "operation"
) -> T:
    """
    ...
    for attempt in range(config.max_attempts):
        try:
            ...
            if attempt == config.max_attempts - 1:
                print(
                    f"[Retry] {operation_name}: All {config.max_attempts} attempts exhausted. Last error: {type(e).__name__}: {e}"
                )
                raise
            ...
            print(
                f"[Retry] {operation_name}: Attempt {attempt + 1}/{config.max_attempts} failed ({type(e).__name__}: {e}). Retrying in {delay:.2f}s..."
            )
            time.sleep(delay)
