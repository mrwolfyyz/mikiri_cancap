...
def on_job_updated(event: CloudEvent) -> None:
    ...
    """
    print(f"[OriginationReportGenerator] Function triggered!")
    job_id = None
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print(f"[OriginationReportGenerator] Calling generate_identity_report() with simplified=True")
            md_gen.generate_identity_report(report_data, borrower_name, output_dir, company_domain=company_domain, enrichment_data=enrichment_data, simplified=True)
            ...
            if len(output_files) != 1 or not output_files[0].name.startswith("Identity___"):
                print(f"[OriginationReportGenerator] WARNING: Expected only Identity report, but found {len(output_files)} files")
            
            print(f"[OriginationReportGenerator] ✓ Identity report generated")
            ...
            try:
                if identity_file.exists():
                    ...
                    print(f"[OriginationReportGenerator] ✓ Read Identity markdown ({len(markdown_reports['identity'])} chars)")
                else:
            ...
            if markdown_reports:
                ...
                print(f"[OriginationReportGenerator] ✓ Stored markdown reports in Firestore for chat feature")
            
            job_ref.set(update_data, merge=True)
            
            print(f"[OriginationReportGenerator] ✓ Updated Firestore: status=complete, reports_generated=true")
