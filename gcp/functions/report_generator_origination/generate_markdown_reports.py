...
from report_utils import (
    slugify, geocode_address, generate_street_view_url,
    load_disposable_email_blocklist, is_disposable_email_domain,
    get_domain_registration_date, get_gravatar_profile, check_domain_mx_records,
    normalize_address, generate_google_search_url, generate_phone_variations,
    generate_google_search_url_for_phone, generate_google_search_url_for_email,
)
    ...
    if company_domain:
        ...
        if company_domain:
            ...
            if enrichment_data and enrichment_data.get('domains') and company_domain in enrichment_data['domains']:
                ...
                from concurrent.futures import ThreadPoolExecutor, as_completed
                print(f"[Identity Report] WARNING: Falling back to inline lookups for company domain {company_domain} (domain_enrichment data missing)")
                with ThreadPoolExecutor(max_workers=2) as executor:
