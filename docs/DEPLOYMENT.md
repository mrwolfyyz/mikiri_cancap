...
## Skiptrace search history (cross-user visibility)

The skiptrace search history feature lets any authenticated internal user list, filter, view, and CSV-export every skiptrace investigation performed by anyone in the customer organization. This is a deliberate feature for operational handoff and quality review, accepted by the customer in writing.

**This feature is only safe when all three of the following are true:**

1. `REQUIRE_SSO=true` (so only Google-Workspace-verified identities can call the API)
2. `ALLOWED_EMAIL_DOMAINS` is set to the customer's corporate domain(s) only
3. The customer's IdP deprovisions departing employees promptly

Without all three, the cross-user widening is a significantly larger exposure than the customer agreed to.

### Required environment variables (api_gateway function)

| Variable | Source | Notes |
|---|---|---|
| `REQUIRE_SSO` | `terraform.tfvars` (`enable_sso = true`) | Must be `true` in any environment using this feature. |
| `ALLOWED_EMAIL_DOMAINS` | `terraform.tfvars` (`allowed_email_domains = ["yourcorp.com"]`) | Only listed domains can authenticate. |
| `HISTORY_TOKEN_SECRET` | Auto-generated by Terraform (`random_password.history_token_secret`) | Signs pagination tokens. Not operator-supplied. **Rotation invalidates in-flight pagination tokens — see "Rotating HISTORY_TOKEN_SECRET" below.** |
| `JOB_RETENTION_DAYS` | `terraform.tfvars` (`job_retention_days = 7`) | TTL for job documents. Minimum 1. Lower values reduce data exposure but shorten the search-history window. Default 7. |

### Quotas

| Endpoint | Quota | Failure mode |
|---|---|---|
| `GET /jobs/history` | 120 requests / hour / user | Fails open on Firestore counter outage. |
| `GET /jobs/history/export.csv` | 10 / hour / user, 50 / 24-hour / user, 5000 rows / export | Fails **closed** on Firestore counter outage. |

When a quota is hit, a `RATE_LIMITED` log line is emitted; surface scraping attempts via:

```
resource.type="cloud_function" textPayload:"[ApiGateway]" textPayload:"RATE_LIMITED"
```

### Auditing cross-user access

Every history list, CSV export, result view, and feedback read/write emits a log line that records the viewer's uid, the owner uid(s) of records returned, the job_id(s), and a `cross_user` boolean or count. To find all cross-user access in a window:

```
resource.type="cloud_function" resource.labels.function_name="api_gateway" textPayload:"cross_user=true"
```

### TTL behaviour

The `jobs` collection has a Firestore TTL policy on `expire_at`. Documents are written with `expire_at = created_at + JOB_RETENTION_DAYS`. Changing `JOB_RETENTION_DAYS` affects only *newly-created* jobs — existing documents keep their original `expire_at`. To purge older data faster after a TTL reduction, run a one-shot Firestore deletion script.

### Rotating `HISTORY_TOKEN_SECRET`

The secret is auto-generated by Terraform's `random_password.history_token_secret`. To rotate:

```bash
cd terraform/environments/prod
terraform taint module.core.random_password.history_token_secret
terraform apply \
  -target='module.core.random_password.history_token_secret' \
  -target='module.core.google_cloudfunctions2_function.api_gateway' \
  -target='module.core.google_storage_bucket_object.api_gateway'
```

**Impact:** all in-flight history pagination tokens become invalid. Users navigating past page 1 of search history will receive `400 Invalid history page token` and need to restart from the first page. Plan rotation outside business hours.
