...
### Firestore
The platform's primary datastore, holding job records, aggregated investigation results, Markdown reports, and per-job chat message history. Firestore security rules restrict client reads of job documents to the owning user (`user_id` matches Firebase UID). TTL policies (Terraform: `google_firestore_field` with `ttl_config` on `expire_at`) apply to the `jobs` and `chat_messages` collection groups; new jobs receive `expire_at` at creation, and report generators refresh it on completion. Deleting a job document via TTL does not automatically delete its `chat_messages` subdocuments—both collection groups have TTL on `expire_at` for retention alignment.
